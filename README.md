...
**`GET /api/response-frequency-analysis`** — query params `condition`, `treatment`, `sample_type` (all required strings, e.g. `melanoma`/`miraclib`/`PBMC`). Compares responding vs. non-responding subjects with the given condition/treatment, restricted to samples of the given type:

```json
{
  "methodology": "Significance is determined by a Mann-Whitney U test comparing responders to non-responders, using one value per subject (the median across that subject's own samples) rather than one value per sample, since the test assumes independent observations. A population is flagged significant at p < 0.01 -- a Bonferroni-corrected threshold (0.05 divided by 5, the number of populations tested at once) rather than the usual 0.05, to control the chance of a false positive across all populations tested together.",
  "boxplots": [
    {
      "population": "b_cell",
...
`boxplots` always includes every population with data for both response groups — unconditionally, not filtered by significance (5 entries against the real dataset) — since the spec asks for a boxplot per population as a visualization requirement, separate from reporting which ones are significant. `statistic` is the comparable number behind `significant` (here, a p-value) — not a raw test statistic like Mann-Whitney U's own U value, whose scale depends on the two groups' sizes and isn't comparable across populations. `methodology` explains in plain language how `significant` was determined; the frontend displays it as-is rather than hardcoding an explanation of whichever test happens to be active.

**Significance testing is pluggable, not hardcoded to Mann-Whitney U.** `backend/crunching/significance.py` defines the swap point: a `SignificanceMethod` bundles a `name`, a `description` (the text that becomes the API's `methodology` field), and a `test` function `(list[float], list[float]) -> SignificanceResult` (a `statistic` plus a `significant` bool). `backend/crunching/mann_whitney_significance.py` is the concrete implementation currently used (wired up in `backend/previsualizing/response_frequency_analysis.py`'s `SIGNIFICANCE_METHOD` constant — swapping methods is a one-line change there). Two methodology decisions are baked into that implementation, both deliberate:
- **One value per subject, not per sample.** Mann-Whitney U assumes independent observations. melanoma+miraclib+PBMC is 656 distinct subjects across 1,968 samples (repeated measures per subject), so testing on raw per-sample values would violate that assumption and bias p-values toward appearing more significant than they really are. `backend/crunching/response_frequencies.py`'s `get_response_subject_frequencies` collapses each subject's samples to their median first — a separate function from `get_response_frequencies` (used for the boxplots' five-number summaries, which don't need independent observations since they're descriptive, not inferential).
- **Bonferroni-corrected alpha.** Testing all 5 populations at once with an uncorrected alpha=0.05 each gives roughly a 23% chance of at least one false "significant" result, not 5%. `mann_whitney_significance.py` uses `0.05 / 5 = 0.01` instead.

Against the real dataset, **zero populations reach significance** at that corrected threshold (`cd4_t_cell` is closest at p≈0.014 — significant at an uncorrected 0.05, but not at the corrected 0.01). That's a real, honest result, not a bug: a properly-controlled test can validly conclude "no significant difference detected."
...
`frontend/` — Vite + React + TypeScript, three pages: `src/pages/Dashboard.tsx` (the cell-frequency summary table, `src/components/CellFrequencyTable.tsx`, from `GET /api/cell-frequencies`), `src/pages/ResponseAnalysis.tsx` (responder vs. non-responder analysis for melanoma/miraclib/PBMC, from `GET /api/response-frequency-analysis`), and `src/pages/SamplesExplorer.tsx` (a general sample browser/query form, from `GET /api/samples`).

- **Navigation:** `App.tsx` switches between the pages with plain `useState`, not a router. `frontend/dist` is served via FastAPI's `StaticFiles(html=True)` with no SPA fallback for sub-paths, so a real path-based route (e.g. `/response-analysis`) would 404 on direct navigation or a refresh unless the backend also grew a catch-all route to `index.html`. A handful of pages doesn't justify that yet — worth reconsidering (a router, plus the backend fallback) if this keeps growing.
- **Sample Explorer page:** one form field per `GET /api/samples` query param (condition/treatment/sample type as free-text, time-from-treatment as a 0/7/14/any select, grouping as project/subject, and a subject-grouping select for sex/response that only appears when grouping is "subject" — mirrors the backend's own validation client-side, so submitting without it shows an inline error instead of a raw 422). Fetches only on explicit Submit. `src/components/SamplesResultTable.tsx` renders `samples` in exactly the order the API returned them (`buildBlocks` only *segments* the existing order into consecutive same-group runs — never re-sorts) as a collapsible accordion: each project (grouping=project) or sex/response value (grouping=subject) gets one header row showing its `counts` entry, **collapsed by default** so all group headers are visible together without scrolling past every sample row, and click-to-expand reveals that group's actual sample rows. "Export CSV" (`src/csv.ts`) downloads the currently-loaded `samples` array — the full per-sample detail, not the grouping — as a proper CSV (RFC 4180 quoting) via a `Blob` + object URL, no server round-trip.
- **Response Analysis page:** fixed to `condition=melanoma`, `treatment=miraclib`, `sample_type=PBMC` (no controls for these, or for anything else — no inputs on this page at all now that significance is a real statistical test rather than a caller-supplied threshold). Fetches once on mount, no refresh control. Renders one compound box-and-whisker chart per `boxplots` entry unconditionally (`src/components/ResponseBoxplot.tsx` — all 5 populations always, responder and non-responder boxes on the same y-axis so they're directly comparable, each chart captioned with its `statistic` and a significant/not-significant badge), split into two sections — "Statistically Significant" and "Not Statistically Significant" — with an explicit, annotated separator between them ("Below: not statistically significant") so the grouping's meaning is unambiguous even when one section is empty (as it currently is against the real data — see the API section above). The API's `methodology` string is rendered as-is above the charts, so the page's explanation of how significance was computed can't drift out of sync with whichever significance method is actually active on the backend.
...
`tests/conftest.py` holds the shared fixture/helpers every backend test file uses: an `engine` fixture (a scratch SQLite file per test, schema via `metadata.create_all()` — not Alembic, and not the real `teiko.db`) plus `insert_project`/`insert_subject`/`insert_sample` helpers general enough to set up whatever condition/treatment/sample_type/response scenario a test needs. Run with `pytest` from the repo root (needs `backend/requirements.txt` installed; a root-level `conftest.py` makes the `backend` package importable regardless of `pytest`'s own rootdir logic).

`tests/test_cell_frequencies_api.py` has integration tests for `GET /api/cell-frequencies`, using FastAPI's `TestClient` against the scratch DB. `httpx2`, not `httpx`, is what's installed: Starlette's `TestClient` now imports `httpx2` first and only falls back to `httpx` (with a deprecation warning) if `httpx2` isn't installed. No test code imports either package directly — only `fastapi.testclient.TestClient` — so this was a one-line dependency swap. `test_multiple_samples_are_independent_and_ordered` and `test_samples_ordered_by_insertion_not_by_source_id` pin down the ordering `get_cell_frequencies` produces (`Sample.id` — insertion order — then the fixed `POPULATIONS` tuple order per sample): the second test specifically inserts samples whose `source_id` sorts the *opposite* of insertion order, so it can't pass by accident of the two orderings coinciding. The frontend's default (unsorted) table order depends on this staying true, since it does no client-side re-sort of its own.

**Crunching-layer unit tests**, added once the response-frequency-analysis feature settled (earlier crunching/previsualizing logic was deliberately left untested while it was still changing shape — this layer had stopped changing shape by the time these were written):
- `test_response_frequencies_crunching.py` — `get_response_frequencies`/`get_response_subject_frequencies`: filtering by condition/treatment/sample_type, excluding subjects with no response, and the one behavior that distinguishes the two functions — a subject's multiple samples stay as separate per-sample values in one, collapse to a single per-subject median in the other.
- `test_response_frequency_stats.py` — `compute_boxplot_stats`: the five-number summary matches `min`/`max`/`statistics.median`, the single-value-group special case, and that only requested/present population-response combinations appear in the output.
- `test_mann_whitney_significance.py` — `MANN_WHITNEY_U`: `BONFERRONI_ALPHA` is `ALPHA / 5`; clearly-separated vs. identical distributions get the right `significant` value; `statistic` matches calling `scipy.stats.mannwhitneyu` directly (a regression pin); `significant` is exactly `statistic < BONFERRONI_ALPHA`. **Caught a real bug**: `mannwhitneyu`'s return values are `numpy.float64`/produce `numpy.bool_` from the `<` comparison, not native Python `float`/`bool` — `numpy.bool_(False) is False` is `False` in Python, which the tests hit directly (`result.significant is False` failed). Fixed by explicitly casting both in `mann_whitney_significance.py`'s `_test`.
- `test_response_significance.py` — `get_response_significance_analysis`: composition/wiring, not statistics (a stub `SignificanceMethod` stands in for Mann-Whitney U here). Covers populations returned in fixed order, a response group missing entirely excluding all 5 populations, boxplot stats reflecting the real underlying sample data, and — via a spy `SignificanceMethod` — that the significance test actually receives per-subject-collapsed values (one per subject) rather than raw per-sample values, even when a subject contributes multiple samples.

`test_response_frequency_analysis_api.py` is the API-level integration test for `GET /api/response-frequency-analysis` — through the real route → previsualizing → crunching stack, including the actual Mann-Whitney U method (unlike `test_response_significance.py`'s stub). Covers: an unmatched condition/treatment/sample_type returns `200` with `boxplots: []` and a non-empty `methodology`; a missing required query param is a `422`; the now-removed `median_threshold` param is silently ignored rather than rejected (FastAPI's default behavior for unrecognized query params) rather than causing an error; all 5 populations are always present regardless of significance; the JSON shape matches the schema exactly (`population`/`responder`/`non_responder`/`statistic`/`significant`, each `BoxplotStats` with `minimum`/`q1`/`median`/`q3`/`maximum`); and — with 10 responder + 10 non-responder subjects whose `b_cell` percentages are constructed with zero overlap between groups — that population is actually flagged `significant` end-to-end, with `statistic` under `BONFERRONI_ALPHA` and `methodology` naming the test used.
